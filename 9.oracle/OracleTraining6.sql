...
CREATE TABLE MEMBERS (
    ...
    CONSTRAINT UK_MEMBERS_GENDER CHECK (GENDER IN ('M', 'F')),
    CONSTRAINT UK_MEMBERS_GRADE CHECK (MEMBER_GRADE IN ('BRONZE', 'SILVER', 'GOLD', 'VIP', 'VVIP')),
    CONSTRAINT UK_MEMBERS_LOGIN_TYPE CHECK (LOGIN_TYPE IN ('NORMAL','KAKAO', 'NAVER','GOOGLE')),
    CONSTRAINT UK_MEMBERS_SNS_AGREE CHECK (IS_SNS_AGREE IN ('Y', 'N')),
    CONSTRAINT UK_MEMBERS_EMAIL_AGREE CHECK (IS_EMAIL_AGREE IN ('Y', 'N')),
    CONSTRAINT UK_MEMBERS_SNS_ACTIVE CHECK (IS_ACTIVE IN ('Y', 'N')),
    CONSTRAINT UK_MEMBERS_POINTS CHECK (POINTS >=0),
    CONSTRAINT UK_MEMBERS_PURCHASE CHECK (TOTAL_PURCHASE >=0)
);

-- 테이블 및 컬럼 주석
comment on table MEMBERS IS '회원 정보 테이블';
COMMENT ON COLUMN MEBERS.MEMBER_ID IS '회원 고유번호 (PK)';
COMMENT ON COLUMN MEBERS.EMAIL IS '이메일 주소 (로그인ID, UNIQUE)';
COMMENT ON COLUMN MEBERS.PASSWORD IS '비밀번호 (암호화 저장)';
