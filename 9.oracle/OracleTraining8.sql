...
CREATE TABLE TABLE_NOTNULL2(
    LOGIN_ID VARCHAR2(20) CONSTRAINT TBLNL2_LGNID_NN NOT NULL, -- 명명규칙: 테이블명_컬럼명
    LOGIN_PWD VARCHAR2(20)CONSTRAINT TBLNL2_LGNPW_NN NOT NULL,
    TEL       VARCHAR2(20)
);
...
-- 14-9. ALTER TABLE로 NOT NULL제약조건 추가
-- NULL 값이 없는 컬럼에 NOT NULL 제약조건을 추가하시오.
ALTER TABLE TABLE_NOTNULL
MODIFY (TEL NOT NULL);  -- NULL 값이 없으므로
...
MODIFY (TEL CONSTRAINT TBLNN_TEL_NN NOT NULL);  -- 제약조건 이름 지정
...
-- 14-14. UNIQUE 제약 조건을 가진 테이블 생성
CREATE TABLE TABLE_UNIQUE(
    LOGIN_ID    VARCHAR2(20) UNIQUE, -- 로그인 ID: 중복 불가, NULL은 허용
    LOGIN_PWD   VARCHAR2(20) NOT NULL, -- 로그인 패스워드: 필수 입력
    TEL VARCHAR2(20)
);
...
INSERT INTO TABLE_UNIQUE(LOGIN_ID, LOGIN_PWD, TEL)
VALUES('TEST_ID_01', 'PWD01', '010-1234-5678'); -- ORA-00001
...
UPDATE TABLE_UNIQUE
 ...
 WHERE LOGIN_ID IS NULL;            -- ORA-00001: 무결성 제약 조건에 위배
 
 -- 14-21. 명시적 이름의 UNIQUE 제약조건
 CREATE TABLE TABLE_UNIQUE2(
    LOGIN_ID    VARCHAR2(20) CONSTRAINT TBLUNQ2_LGNID_UNQ UNIQUE, -- 로그인 ID: 중복 불가, NULL은 허용
    LOGIN_PWD   VARCHAR2(20) CONSTRAINT TBLUNQ2_LGNPW_NN NOT NULL, -- 로그인 패스워드: 필수 입력
    TEL VARCHAR2(20)
);
...
SELECT OWNER,
        ...
        TABLE_NAME          -- 테이블 이름
FROM USER_CONSTRAINTS
WHERE TABLE_NAME LIKE 'TABLE_UNIQUE2';

-- 14-29. PRIMARY KEY 제약조건을 가진 테이블 생성
CREATE TABLE TABLE_PK(
    LOGIN_ID    VARCHAR2(20) PRIMARY KEY, -- 로그인 ID - NOT NULL +UNIQUE + 인덱스 자동 생성
    LOGIN_PWD   VARCHAR2(20) NOT NULL, -- 로그인 패스워드: 필수 입력
    TEL VARCHAR2(20)
);
...
SELECT OWNER,
        ...
        TABLE_NAME          -- 테이블 이름
FROM USER_CONSTRAINTS
WHERE TABLE_NAME LIKE 'TABLE_PK2';

-- 14-32. 명시적 
CREATE TABLE TABLE_PK2(
    LOGIN_ID    VARCHAR2(20) CONSTRAINT TBLPK2_LGNID_PK PRIMARY KEY, -- 명시적 이름
    LOGIN_PWD   VARCHAR2(20) CONSTRAINT TBLPK2_LGNPW_NN NOT NULL, -- 로그인 패스워드: 필수 입력
    TEL VARCHAR2(20)
);
...
(	"EMPNO" NUMBER(4,0) CONSTRAINT "EMPFK_EMPNO_PK" PRIMARY KEY, 
	...
	"SAL" NUMBER(7,2), 
	"COMM" NUMBER(7,2), 
	"DEPTNO" NUMBER(2,0) CONSTRAINT "EMPFK_DEPTNO_FK" REFERENCES DEPT_FK(DEPTNO) -- 외래키
    );

DESC EMP_FK;

-- 14-41. 부모 테이블에 데이터가 없을 때 자식 테이블 입력
...
-- ORA-02290: 체크 제약조건(SCOTT.TBLCK_LOGINPW_PK)이 위배되었습니다
