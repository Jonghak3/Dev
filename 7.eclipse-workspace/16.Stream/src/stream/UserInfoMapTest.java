...
public class UserInfoMapTest {
	...
	public static void main(String[] args) {
		 ...
		 * ::
		 * 		- 메서드 레퍼런스
		 * 		- 이미 존재하는 메서드를 "참조"해서 람다식 대신 사용하는 문법임
		 */
		userInfos.stream().filter(user -> user.getAge() >= 40)
					.map(UserInfo::getName)		//이름만 추출
//					.forEach(s -> System.out.println(s+" "));
					.forEach(System.out::println);
		
		List<String> names =
		Arrays.asList("김철수", "이영희", "박민수", "최지혜", "김영수");
		...
		System.out.println("------------");
		names.stream().filter(name -> name.startsWith("김"))	//중간연산: 조건에 맞는 요소만 통과
			.forEach(System.out::println);					//최종연산: 각 요소를 출력
