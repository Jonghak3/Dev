...
public class Book {
	...
	public String toString() {
		...
		String status = available ? "대출가능": 
			("대출중(대출자: "+borrowedId+", 대출일: "+
					borrowDate.format(formatter)+
					(isOverdue() ? "-- 연체됨" : "")+
					")"
			);
		
		return String.format("ISBN: %s, 제목: %s, 저자: %s, 카테고리: %s, 출판일: %s, 상태: %s", 
				isbn, title, author, category, publishDate.format(formatter), status);
	}
