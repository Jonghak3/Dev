...
public class CollectionSortingLab {
	...
	private static void sortByAge(List<Employee> employees) {
		System.out.println("나이순 정렬 (오른차순) ");
...
//		});
		...
		printEmployee(sorted);	//정렬된 결과 출력
	...
	private static List<Employee> createEmployees() {
		/*
		 * - Date.from(Instant instant)로 Date 객체 생성
		 * 	- 시간대, 시점 정보가 필요한 경우가 있어서 LocalDate.of()로 날짜 생성
		 */
		return Arrays.asList(
				new Employee("김개발", 28, "개발팀", 45000000, Date.from(LocalDate.of(2022, 3, 13)
						.atStartOfDay(ZoneId.systemDefault()).toInstant())),
				new Employee("박기획", 32, "기획팀", 50000000, Date.from(LocalDate.of(2020, 6, 10)
						.atStartOfDay(ZoneId.systemDefault()).toInstant())),
				new Employee("이디자인", 25, "디자인팀", 40000000, Date.from(LocalDate.of(2023, 1, 20)
						.atStartOfDay(ZoneId.systemDefault()).toInstant())),
				new Employee("최마케팅", 30, "마케팅팀", 48000000, Date.from(LocalDate.of(2021, 9, 5)
						.atStartOfDay(ZoneId.systemDefault()).toInstant())),
				new Employee("정개발", 26, "개발팀", 42000000, Date.from(LocalDate.of(2023, 4, 12)
						.atStartOfDay(ZoneId.systemDefault()).toInstant())),
				new Employee("한디자인", 28, "디자인팀", 43000000, Date.from(LocalDate.of(2019, 12, 3)
						.atStartOfDay(ZoneId.systemDefault()).toInstant()))
				);
	}
}
